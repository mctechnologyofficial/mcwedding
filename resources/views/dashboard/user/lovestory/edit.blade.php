@extends('app')
@section('title', 'Ubah Cerita Cinta')
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-lg-12 text-center">
                            <img id="image" src="{{ $lovestory->image == null ? asset('dashboard/assets/img/users/1.jpg') : asset($lovestory->image) }}" alt="" class="img-thumbnail w-25">
                        </div>
                    </div>

                    <form action="{{ route('user.lovestory.update', $lovestory->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label for="exampleInputPassword1">Tanggal Cerita</label>
                            <input type="date" name="date_story" class="form-control" id="exampleInputPassword1" aria-describedby="emailHelp" value="{{ $lovestory->date_story }}">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">Judul Cerita</label>
                            <input type="text" name="title" class="form-control" id="exampleInputPassword1" aria-describedby="emailHelp" value="{{ $lovestory->title }}">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">Cerita</label>
                            <textarea name="story" class="form-control" cols="30" rows="10">{{ $lovestory->story }}</textarea>
                        </div>
                        <div class="form-group">
